...
def delete_sql_full(gk_val, schema: dict, meta: dict) -> dict:
    ...
    if not os.path.exists(SQLITE_FILE):
        print(f"  [WARN] SQLite file '{SQLITE_FILE}' not found — SQL delete skipped.")
        return {}
    ...
    try:
        ...
        for table in schema["child_tables"]:
            cur = conn.execute(
                f"DELETE FROM {table} WHERE {global_key} = ?", [gk_val]
            )
        ...
        cur  = conn.execute(
            f"DELETE FROM {main} WHERE {global_key} = ?", [gk_val]
        )
    ...
    try:
        ...
        if ftype in ("object", "array"):
            ...
            try:
                ...
                # Fallback: set all values to NULL (SQLite < 3.35)
