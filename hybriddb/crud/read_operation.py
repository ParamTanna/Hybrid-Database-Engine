...
from hybriddb.storage.audit_store import get_entries
...
AUDIT_FIELDS = ("created_at", "last_updated_at")


def _coerce_scalar_for_output(field_name: str, expected_type: str, value):
    if expected_type == "string":
        return str(value)
    if expected_type == "boolean":
        if isinstance(value, bool):
            return value
        raise ValueError(f"Field '{field_name}' expects boolean, got {type(value).__name__} ({value!r}).")
    if expected_type == "int":
        if isinstance(value, bool):
            raise ValueError(f"Field '{field_name}' expects int, got bool ({value!r}).")
        try:
            return int(value)
        except Exception:
            raise ValueError(f"Field '{field_name}' expects int, got {type(value).__name__} ({value!r}).")
    if expected_type == "float":
        if isinstance(value, bool):
            raise ValueError(f"Field '{field_name}' expects float, got bool ({value!r}).")
        try:
            return float(value)
        except Exception:
            raise ValueError(f"Field '{field_name}' expects float, got {type(value).__name__} ({value!r}).")
    return value


def _coerce_record_by_meta(record: dict, meta: dict) -> dict:
    fields = meta.get("fields", {})
    child_map: dict[str, dict[str, str]] = {}
    for fname, fmeta in fields.items():
        parent = fmeta.get("parent")
        if parent:
            child_map.setdefault(parent, {})[fname.split(".")[-1]] = fname

    def _coerce_field(field_name: str, value):
        fmeta = fields.get(field_name, {})
        ftype = fmeta.get("type", "string")

        # Optional fields may be absent/null in merged records; keep nulls as-is.
        if value is None:
            return None

        # Type-widened fields hold mixed types on purpose — return as stored.
        if fmeta.get("type_widened"):
            return value

        if ftype not in ("object", "array"):
            return _coerce_scalar_for_output(field_name, ftype, value)
        ...
        if ftype == "object":
            ...
            for leaf, child_full in cmap.items():
                if leaf in out:
                    out[leaf] = _coerce_field(child_full, out[leaf])
            return out
        ...
        for item in value:
            if isinstance(item, dict) and cmap:
                ...
                for leaf, child_full in cmap.items():
                    if leaf in new_item:
                        new_item[leaf] = _coerce_field(child_full, new_item[leaf])
    ...
    for fname, fmeta in fields.items():
        if fmeta.get("level") != 0:
            continue
        if fname not in out_record:
            continue
        out_record[fname] = _coerce_field(fname, out_record[fname])
    return out_record
...
def query_sql(meta: dict, sql_tables: dict[str, list], where_sql: dict) -> dict:
    """
    Query PostgreSQL for the requested columns.
    ...
    if not sql_db.health_check():
        print("  [WARN] SQL backend not reachable.")
        return {}
    ...
    try:
        # If where targets SQL fields like email (without global_key), resolve
        # candidate global keys from the main table first so child-table reads
        # do not fan out to unrelated records.
        candidate_keys = None
        if where_sql and global_key not in where_sql and main_table in km:
            main_cols = set(km.get(main_table, {}).get("columns", []))
            seed_where = {k: v for k, v in where_sql.items() if k in main_cols}
            if seed_where:
                clauses = []
                params = []
                for k, v in seed_where.items():
                    if isinstance(v, list):
                        ...
                        params.append(v)
                seed_sql = f"SELECT {global_key} FROM {main_table} WHERE " + " AND ".join(clauses)
                seed_cur = conn.execute(seed_sql, params)
                seed_rows = seed_cur.fetchall()
                candidate_keys = [r[global_key] for r in seed_rows]
                if not candidate_keys:
                    return {}

        for table, cols in sql_tables.items():
            # Only select columns that actually exist in this table
            table_cols = sql_db.column_names(conn, table)
            ...
            if candidate_keys is not None:
                applicable_where[global_key] = candidate_keys
            ...
            if applicable_where:
                clauses = []
                for k, v in applicable_where.items():
                    if isinstance(v, list):
                        if not v:
                            where_str = " WHERE 1=0"
                            params = []
...
def query_buffer(meta: dict, buffer_fields: list[str], where_buf: dict) -> dict:
    ...
    for rec in records:
        ...
        for k, v in where_buf.items():
            _top = rec.get(k)
            rec_val = _top if _top is not None else rec.get("unknown_top", {}).get(k)
            if rec_val != v:
                match = False
                break
        if not match:
...
def _path_get(record: dict, dotted_key: str):
    cur = record
...
def _matches_where(record: dict, where: dict) -> bool:
...
def _project_requested(records: list[dict], requested: list[str]) -> list[dict]:
    requested_set = set(requested)
    out = []
    for rec in records:
        out.append({k: v for k, v in rec.items() if k in requested_set})
    return out
...
def execute_read(query: dict, meta: dict) -> list[dict]:
    ...
    wildcard_requested = "*" in requested
    ...
    if wildcard_requested:
        ...
        requested = top_level
        original_requested = list(top_level) + list(AUDIT_FIELDS)

    # Include where keys and global key in fetch set so final strict where
    # filtering can be applied after backend merge.
    global_key = meta.get("global_key")
    requested_for_fetch = list(
        dict.fromkeys(requested + list(where.keys()) + ([global_key] if global_key else []))
    )

    # ── Step 1: resolve fields to backends ───────────────────────────────
    resolved = resolve_fields(requested_for_fetch, meta)
    ...
    # ── Step 4: merge + strict where filter + projection ─────────────────
    merged = merge_results(sql_result, mongo_result, buf_result)

    if where:
        merged = [rec for rec in merged if _matches_where(rec, where)]

    if merged:
        merged = [_coerce_record_by_meta(rec, meta) for rec in merged]

    audit_to_attach = [
        f for f in AUDIT_FIELDS
        if wildcard_requested or f in original_requested
    ]
    if merged and global_key and audit_to_attach:
        key_values = [rec.get(global_key) for rec in merged if rec.get(global_key) is not None]
        audit_map = get_entries(global_key, key_values)
        for rec in merged:
            gk_val = rec.get(global_key)
            if gk_val is None:
                continue
            audit = audit_map.get(str(gk_val), {})
            for fname in audit_to_attach:
                if fname in audit:
                    rec[fname] = audit[fname]

    return _project_requested(merged, original_requested)
