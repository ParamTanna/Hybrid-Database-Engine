...
def _classify_entity(fname: str, fdata: dict, freq: float,
    ...
    # A field whose type has drifted (mixed int/string/etc.) cannot live in a
    # strict SQL column, so it is permanently pinned to schemaless Mongo —
    # regardless of how frequent it is.
    if fdata.get("type_widened"):
    ...
    if field_type in PRIMITIVE_TYPES:
        # Rare primitive fields are better handled in Buffer.
        if freq < FREQ_RARE:
            return "Buffer", "Buffer"
