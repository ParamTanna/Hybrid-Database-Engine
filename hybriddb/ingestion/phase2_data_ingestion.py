...
def _update_constraints_from_data(meta: dict, seen_values: dict):
    ...
    Updates not_null and unique on every field using observed data,
    while preserving schema-declared unique constraints.

    not_null <- True only when the field appeared in every single record.
    unique   <- True when either:
                1) schema explicitly declares unique=true, or
                2) observed values are all distinct for that field.
    """
    ...
    try:
        with open(SCHEMA_FILE, "r") as _f:
            _raw_schema = _json.load(_f)
    except Exception:
        _raw_schema = {}

    def _schema_unique_paths(fields: dict, parent: str = "") -> set[str]:
        unique = set()
        for name, fdef in fields.items():
            ...
            if fdef.get("unique") is True:
                unique.add(full)
            nested = fdef.get("fields")
            if isinstance(nested, dict):
                unique |= _schema_unique_paths(nested, full)
        return unique

    schema_unique_fields = _schema_unique_paths(
        {k: v for k, v in _raw_schema.items() if k != "global_key"}
    )
    ...
    for fname, fmeta in meta["fields"].items():
        ...
        # Keep schema-declared unique fields enforced even if a sampled batch
        # happens to contain duplicates; schema is the source of truth.
        if fname in schema_unique_fields:
            fmeta["unique"] = True
        else:
            # Containers are not treated as unique scalar values.
            if fmeta.get("type") in ("array", "object"):
                fmeta["unique"] = False
                continue

            seen = seen_values.get(fname, set())
            # Unique when every observed value for this field is distinct.
            fmeta["unique"] = (occ > 0 and len(seen) == occ)
