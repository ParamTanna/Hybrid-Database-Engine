...
# ── Pipeline phases ─────────────────────────────────────────────────────────
from hybriddb.ingestion.phase1_schema_registration import build_metadata

from hybriddb.ingestion.phase2_data_ingestion import (
    process_record,
...
from hybriddb.storage.db_init import (
    setup_postgres,
    setup_mongo,
    insert_sql,
    insert_mongo,
...
from hybriddb.storage.audit_store import clear_store as _clear_audit_store
...
STREAM_BASE   = paths.STREAM_BASE
DEFAULT_COUNT = paths.DEFAULT_COUNT
...
def _run_db_init(meta: dict):
    ...
    from hybriddb.storage.buffer_store import staging_load as _stg_load, flush_staging_to_mongo as _flush
    staging = _stg_load()
    ...
    while True:
        ...
        if op == "read":
            ...
            if result.get("success"):
                print("\n  Result:")
                print(json.dumps(result.get("data") or [], indent=4, default=str))
            else:
                print(f"\n  FAILED: {result.get('message')}")

        elif op == "insert":
            result = tc.execute(query)
            print(f"\n  {'SUCCESS' if result.get('success') else 'FAILED'}: {result.get('message')}")
...
def menu_reset():
    ...
    print(f"    - PostgreSQL '{paths.PG_DB}' public schema (all SQL tables)")
    ...
    try:
        ...
        print(f"  Postgres : all tables in '{paths.PG_DB}' public schema dropped")
    except Exception as exc:
        print(f"  Postgres : drop failed — {exc}")
    ...
    removed_audit = _clear_audit_store()
    if removed_audit:
        print("  Audit    : record_audit_store.json deleted")
    else:
        print("  Audit    : record_audit_store.json not found — skipped")
...
def _status_line() -> str:
    ...
    try:
        if sql_db.health_check():
            conn = sql_db.connect(autocommit=True)
            try:
                tbls = sql_db.list_tables(conn)
            finally:
                conn.close()
            parts.append(f"sql=ok({','.join(tbls)})" if tbls else "sql=empty")
        else:
            parts.append("sql=down")
    except Exception:
...
def run_menu():
    """Interactive hub entry point (console-script `hybriddb`)."""
    ...
    while True:
        ...
        if action:
            ...
            print(f"\n  Unknown option '{choice}' — enter 1-4 or 0 to exit.")
