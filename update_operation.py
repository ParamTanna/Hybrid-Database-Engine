...
def build_merged_record(old_record: dict, query: dict, meta: dict) -> dict:
    ...
    if entity is None:
        ...
        # For the insert step we only push the matched (updated) items to
        # avoid duplicate-key conflicts with unchanged rows still in the DB.
        updated_items = [
            item for item in new_items
            if all(item.get(k) == v for k, v in extra_where.items())
        ]
        return {global_key: gk_val, entity: updated_items}
