    ...
    def insert_record(
        ...
        try:
            ...
            if self.mongo_available:
                ...
                # MongoDB unavailable — fall back to storing mongo-classified fields
                # as JSON text columns in the 'records' SQL table so no data is lost.
                fallback_row: dict[str, Any] = {}
                for k, v in record.items():
                    p = field_placement.get(k, {})
                    if p.get("backend") == "mongo":
                        fallback_row[k] = self._value_to_sql(v)

                if fallback_row:
                    with self.sql_engine.connect() as fb_conn:
                        existing_cols = {
                            row[1] for row in fb_conn.execute(
                                text('PRAGMA table_info("records")')
                            ).all()
                        }
                        for col in list(fallback_row.keys()):
                            if col not in existing_cols:
                                try:
                                    fb_conn.execute(text(
                        ...
                        if fallback_row:
                            fallback_row[config.JOIN_KEY] = sys_ingested_at
                            set_clauses = ", ".join(
                                f'"{k}" = :{self._safe_key(k)}' for k in fallback_row if k != config.JOIN_KEY
                            )
                            safe_fb = {self._safe_key(k): v for k, v in fallback_row.items()}
                            fb_conn.execute(
                                text(f'UPDATE "records" SET {set_clauses} WHERE "{config.JOIN_KEY}" = :{self._safe_key(config.JOIN_KEY)}'),
                                safe_fb,
                            )
                            fb_conn.commit()
                            print(f"  [INSERT] MongoDB unavailable — fell back to SQL for fields={list(fallback_row.keys())}")
