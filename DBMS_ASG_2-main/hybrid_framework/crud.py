...
def _coerce_mongo_filter(filters: dict) -> dict:
    """
    The CLI delivers all filter values as strings.  MongoDB is strongly typed,
    so {"age": "24"} will never match a document where age is stored as 24 (int).
    Try to coerce each string value to int then float; leave it as a string if
    ...
    def execute_read(
        ...
        try:
            with self.sql_engine.connect() as conn:
                ...
                # ── Step 0: pre-filter via Mongo when filters contain mongo fields ──
                # Query all relevant mongo collections first to get the set of
                # JOIN_KEY timestamps that match the mongo-side filters.  These
                # timestamps are then used to restrict the SQL query so we never
                # return records that don't match a mongo-side filter.
                mongo_ts_filter: set[str] | None = None
                mongo_filter_fields = {
                    k: v for k, v in filters.items()
                    if field_placement.get(k, {}).get("backend") == "mongo"
                }
                mongo_filter_fields = _coerce_mongo_filter(mongo_filter_fields)
                if mongo_filter_fields and self.mongo_available:
                    matching_ts: set[str] = set()
                    all_mongo_colls = dict(mongo_collections)
                    if "main_documents" not in all_mongo_colls:
                        all_mongo_colls["main_documents"] = {"strategy": "embed", "embedded_fields": []}
                    for coll_name in all_mongo_colls:
                        try:
                            docs = list(self.mongo_db[coll_name].find(mongo_filter_fields, {config.JOIN_KEY: 1, "_id": 0}))
                            for d in docs:
                            ...
                            pass
                    mongo_ts_filter = matching_ts
                    print(f"  [READ] step0 mongo pre-filter: {len(matching_ts)} matching timestamps for filters={mongo_filter_fields}")
                ...
                if "records" not in existing_tables:
                    ...
                    # Restrict to timestamps matched by mongo pre-filter (Step 0)
                    if mongo_ts_filter is not None:
                        if not mongo_ts_filter:
                            # No mongo docs matched — result is definitely empty
                            print("  [READ] step0 mongo pre-filter matched 0 docs — skipping SQL query")
                            mongo_ts_filter = set()  # leave sql_results empty
                        else:
                            ts_list = list(mongo_ts_filter)
                            ts_ph   = ", ".join(f":_mts{i}" for i in range(len(ts_list)))
                            for i, ts in enumerate(ts_list):
                                params[f"_mts{i}"] = ts
                            where_clauses.append(f'"{config.JOIN_KEY}" IN ({ts_ph})')

                    if mongo_ts_filter is not None and not mongo_ts_filter:
                        rows = []  # skip SQL entirely
                    else:
    ...
    def execute_update(
        ...
        try:
            with self.sql_engine.connect() as conn:
                ...
                # Pre-filter via Mongo for any filter fields that live in mongo,
                # then restrict the SQL WHERE to only matching timestamps.
                mongo_filter_fields = {
                    k: v for k, v in filters.items()
                    if field_placement.get(k, {}).get("backend") == "mongo"
                }
                # Coerce string values to int/float so MongoDB type matching works
                # (the CLI always delivers filter values as strings).
                mongo_filter_fields = _coerce_mongo_filter(mongo_filter_fields)
                if mongo_filter_fields and self.mongo_available:
                    matching_ts: set[str] = set()
                    all_mongo_colls = dict(mongo_collections)
                    if "main_documents" not in all_mongo_colls:
                        all_mongo_colls["main_documents"] = {}
                    for coll_name in all_mongo_colls:
                        try:
                            docs = list(self.mongo_db[coll_name].find(
                                mongo_filter_fields, {config.JOIN_KEY: 1, "_id": 0}
                            ))
                            for d in docs:
                                ts = d.get(config.JOIN_KEY)
                                if ts:
                            ...
                            pass
                    print(f"  [UPDATE] mongo pre-filter matched {len(matching_ts)} timestamps")
                    if matching_ts:
                        ts_list = list(matching_ts)
                        ts_ph   = ", ".join(f":_uts{i}" for i in range(len(ts_list)))
                        for i, ts in enumerate(ts_list):
                            rec_params[f"_uts{i}"] = ts
                        rec_where.append(f'"{config.JOIN_KEY}" IN ({ts_ph})')
                    else:
                        # No mongo docs matched — nothing to update
                        print("  [UPDATE] mongo pre-filter matched 0 docs — nothing to update")
                        return {"updated_sql_rows": 0, "updated_mongo_docs": 0}
